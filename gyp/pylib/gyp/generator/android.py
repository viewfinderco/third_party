...
generator_default_variables = {
  ...
  'SHARED_INTERMEDIATE_DIR': '$(gyp_intermediate_dir)',
  'PRODUCT_DIR': '$(gyp_intermediate_dir)',
...
def RenameFileIfDifferent(source_file, dest_file):
  if os.access(dest_file, os.F_OK):
    assert os.access(dest_file, os.R_OK), 'Could not read %s' % dest_file
    source_contents = open(source_file, 'r').read()
    dest_contents = open(dest_file, 'r').read()
    if source_contents == dest_contents:
      os.unlink(source_file)
      return

  # Destination does not exist or is different: overwrite.
...
class AndroidMkWriter(object):
  ...
  def Write(self, qualified_target, relative_target, base_path, output_filename,
    ...
    tmp_filename = '%s.tmp' % output_filename

    self.fp = open(tmp_filename, 'w')
    ...
    self.WriteLn('gyp_intermediate_dir := $(abspath $(LOCAL_PATH)/%s)' % self.path)
    self.WriteLn()
    ...
    self.WriteLn('GYP_TARGET_DEPENDENCIES += $(GYP_GENERATED_OUTPUTS)')
    self.WriteLn()
    ...
    RenameFileIfDifferent(tmp_filename, output_filename)
  ...
  def WriteSources(self, spec, configs, extra_sources):
    ...
    local_files.extend(final_generated_sources)
  ...
  def LocalPathify(self, path):
    ...
    # TODO(marc): figure out if commenting this check out is really legit.
...
def GenerateOutput(target_list, target_dicts, data, params):
  ...
  tmp_makefile_path = '%s.tmp' % makefile_path
  root_makefile = open(tmp_makefile_path, 'w')
  ...
  RenameFileIfDifferent(tmp_makefile_path, makefile_path)
